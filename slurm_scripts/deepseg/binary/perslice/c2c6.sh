...
# Generate the output values for vertebral labelling (needed to isolate the vertebral ranges)
...
PER_SLICE_OUT_NAME="deepsegv2_slice_metrics.csv"
...
if [ ! -f "$PER_SLICE_OUT_FILE" ]; then
  ...
  sct_process_segmentation -i "$SEG_FILE" -vert 2:6 -vertfile "$VERT_FILE" -perslice 1 -o "$PER_SLICE_OUT_NAME"
  ...
  printf "\n"
  echo "Per-slice metric file already exist, skipping"
fi
